...
final class Loader
{
    ...
    private static function getFileName($s_className)
    {
        ...
        $s_className = preg_replace_callback('/([A-Z])/s',
            function ($route) {
    ...
    public static function inject($s_className, $a_arguments = array())
    {
        ...
        if ((substr($s_fileName, 0, 5) == 'core/') && (file_exists(WEB_ROOT . str_replace('core/',
                        'includes/', $s_fileName) . '.inc.php') || file_exists(WEB_ROOT . str_replace('core/',
                        'includes/', $s_fileName) . '.php'))) {
    ...
    {
        if (substr($s_filename, 0, 8) == 'youconix') {
            $s_filename = 'vendor' . DS . $s_filename;
        }
        ...
        } elseif ($service_File->exists($s_filename)) {
            $s_file = $service_File->readFile($s_filename);
        } elseif ($service_File->exists(str_replace('.inc.php', '.php', $s_filename))) {
    ...
    private static function findUses($file)
    {
        ...
        foreach ($uses AS $use) {
            foreach ($use AS $line) {
                ...
                self::$a_uses[trim($parts[1])] = trim($parts[0]);
            }
        }
        ...
        foreach ($uses AS $use) {
            foreach ($use AS $line) {
                ...
                self::$a_uses[trim($className)] = '\\'.$line;
            }
        }
    }
