 ...
 * @since 1.0
 */
class Mysqli extends \youconix\core\ORM\database\GeneralDAL
{
  ...
  {
    \Profiler::profileSystem('core/ORM\database/Mysqli.inc.php',
			     'Connecting to database');
    $this->bo_connection = false;

    $s_type = $this->settings->get('settings/SQL/type');
    $this->connection($this->settings->get('settings/SQL/' . $s_type . '/username'),
					   $this->settings->get('settings/SQL/' . $s_type . '/password'),
			     $this->settings->get('settings/SQL/' . $s_type . '/database'),
			     $this->settings->get('settings/SQL/' . $s_type . '/host'),
			     $this->settings->get('settings/SQL/' . $s_type . '/port'));
			     ...
			     $s_host = '127.0.0.1', $i_port = -1)
  {
    ...
    /* connect to the database */
    if ($i_port == - 1 || $i_port == '') {
      $this->obj_connection = new \mysqli($s_host, $s_username, $s_password,
					  $s_database);
    } else {
      $this->obj_connection = new \mysqli($s_host, $s_username, $s_password,
					  $s_database, $i_port);
   ...
   * @throws DBException if no SELECT-query was executed
   */
  ...
  public function fetch_row()
  {
    ...
    while ($this->obj_query->fetch()) {
      $i_field = 0;
      $a_temp = [];
      foreach ($this->a_result as $value) {
  ...
  {
    $a_rows = $this->fetch_row();
    if (array_key_exists($i_row, $a_rows) || !array_key_exists($i_column,
							       $a_rows[$i_row])) {
      throw new \DBException('Can not fetch column number ' . $i_column . ' from result row ' . $i_row);
    }

    return $a_rows[$i_row][$i_column];
  }
