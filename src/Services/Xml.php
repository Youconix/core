 ...
 * @since 1.0
 */
class Xml extends AbstractService
{
   ...
   */
  public function createDocument($s_encoding = 'iso-8859-1',
				 $bo_skipXPath = false)
  {
  ...
  public function emptyPath($s_path)
  {
    ...
    if ($a_return instanceof \DOMNodeList) {
      foreach ($a_return[0]->childNodes as $child) {
	$a_return[0]->removeChild($child);
      }
    } else {
      $this->set($s_path, '');
    }
  ...
  public function set($s_path, $s_content)
  {
    ...
    $s_realPath = $this->getRealPath($s_path);

    $a_data = explode('/', $s_realPath);
    $s_name = end($a_data);
    $a_return = $this->obj_document->query("//" . $s_realPath);

    if ($a_return->length < 1) {
      /* Part not found */
      $this->add($s_path, $s_content);
      return;
    }
