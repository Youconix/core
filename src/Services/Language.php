 ...
 * @since 1.0
 */
abstract class Language extends \youconix\Core\Services\AbstractService implements \LanguageInterface
{
    ...
    /** @var string */
    protected $fallbackLanguageCode;
    ...
    /** @var string */
    protected $mainStartTag = 'LanguageInterface';

    public function __construct(\youconix\Core\Services\File $file, \ConfigInterface $config)
    {
        $this->file = $file;

        $this->languageCode = $config->getLanguage();
        $this->fallbackLanguageCode = $config->getSettings()->get('settings/fallbackLanguage');
        $this->cacheDir = $config->getCacheDirectory();

        $this->mainLanguage = $this->readLanguage($this->languageCode);
        if (!empty($languageFallback)) {
            $this->fallbackLanguage = $this->readLanguage($this->fallbackLanguageCode);
        }
  ...
  /**
    ...
    public static function text($key)
    {
        if (is_null(Language::$_instance)) {
            Language::$_instance = \Loader::inject('\LanguageInterface');
        }
