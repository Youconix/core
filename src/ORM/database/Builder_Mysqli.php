...
class Builder_Mysqli implements \Builder
{
  ...
  {
    \Profiler::profileSystem('Core/ORM/database/Builder_Mysql.inc.php',
   ...
   */
  public function order($s_field1, $s_ordering1 = 'ASC', $s_field2 = '',
                        $s_ordering2 = 'ASC')
  {
  ...
  public function getResult()
  {
    ...
    foreach ($a_query['types'] as $s_field => $type) {
      switch ($type) {
        case 's':
          $this->service_Database->bindString($s_field,
              $a_query['values'][$s_field]);
          break;

        case 'i':
          $this->service_Database->bindInt($s_field,
              $a_query['values'][$s_field]);
          break;

        case 'f':
          $this->service_Database->bindFloat($s_field,
              $a_query['values'][$s_field]);
          break;

        case 'b':
          $this->service_Database->bindBlob($s_field,
              $a_query['values'][$s_field]);
          break;
      }
    }
  ...
  public function render()
  {
    ...
    $s_command = strtoupper(substr($this->s_query, 0,
            strpos($this->s_query, ' ')));
    ...
    switch ($s_command) {
      ...
      case 'INSERT':
        ...
        if ($this->bo_upsert) {
          ...
          $this->s_resultQuery .= 'ON DUPLICATE KEY UPDATE '.implode(',',
                  $a_updateFields);
        }
        break;
      case 'DELETE':
  ...
  protected function dumpTable($s_table)
  {
    ...
    $s_structure = $this->service_Database->describe(DB_PREFIX.$s_table, false,
        true);
...
abstract class QueryConditions_Mysqli
{
  ...
  private function bind($s_field, $value, $s_glue, $s_key, $s_type)
  {
    ...
    $counter = 0;
    while( array_key_exists($s_field.'_'.$counter, $this->a_values) ){
      $counter++;
    }
...
class Create_Mysqli implements \Create
{
   ...
   */
  public function addRow($s_field, $s_type, $i_length = -1, $s_default = '',
                         $bo_signed = true, $bo_null = false,
                         $bo_autoIncrement = false)
  {
    $s_signed = (($bo_signed) ? ' SIGNED ' : ' UNSIGNED ');
    ...
    $s_autoIncrement = (($bo_autoIncrement) ? ' AUTO_INCREMENT' : '');
