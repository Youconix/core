 ...
 * @since 1.0
 */
class Template extends \youconix\core\services\Service implements \Output, \SplObserver
{
    ...
    protected $a_headerParser;

    protected $a_parts;

    protected $bo_compression = false;

    protected $a_partsUsed = array();

    protected $bo_loaded = false;
     ...
     */
    public function __construct(\youconix\core\services\FileHandler $file, \Config $config, \Cache $cache, \Headers $headers)
    {
        $this->file = $file;
        $this->config = $config;
    ...
    protected function load()
    {
        ...
        $this->s_templateDir = $this->config->getStylesDir();
        
        if (defined('PROCES') || \youconix\core\Memory::isTesting()) {
            return;
        }
        ...
        if (! $this->config->isAjax()) {
            ...
            $s_url = $this->s_templateDir . '/templates/layouts/' . $this->config->getLayout() . '.tpl';
    ...
    {
        \youconix\core\Memory::type('string', $s_view);
        ...
        $s_view = $this->s_templateDir . '/templates/' . $s_template;
    ...
    protected function parse()
    {
        ...
        if (array_key_exists(1, $a_matches)) {
            $s_dir = NIV . $this->s_templateDir . '/templates/';
    ...
    {
        \youconix\core\Memory::type('string', $s_link);
    ...
    {
        \youconix\core\Memory::type('string', $s_javascript);
    ...
    {
        \youconix\core\Memory::type('string', $s_css);
    ...
    {
        \youconix\core\Memory::type('string', $s_meta);
    ...
    public function loadTemplate($s_key, $s_url)
    {
        ...
        if (! $this->file->exists(NIV . $this->s_templateDir . '/templates/' . $s_url)) {
            throw new \TemplateException('Can not find template ' . $s_url);
        }
        
        $s_subTemplate = $this->file->readFile(NIV . $this->s_templateDir . '/templates/' . $s_url);
	...
	';
        ...
        $this->s_template = preg_replace("#{+[a-zA-Z]+[a-zA-Z_0-9/]+}+#si", "", $this->s_template);
     ...
     */
    protected function writeBlocks()
    {
...
';
     ...
     *            The start position
     */
    protected function removeBlocks($s_key, $i_start)
    {
     ...
     */
    protected function removeElse($i_start)
    {
     ...
     * @return int The start position if the end tag, -1 if not found
     */
    protected function findIfEnd($i_start, $s_key = 'if')
    {
