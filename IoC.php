...
class IoC
{
  ...
  {
    if ($this->settings->exists('language/type') && $this->settings->get('language/type')
        == 'mo') {
  ...
  protected function detectMailer()
  {
    if (defined('DEBUG') || \youconix\core\Memory::isTesting()) {
      IoC::$a_rules['MailerLib'] = '\youconix\core\mailer\PHPMailerDebug';
    } else {
      IoC::$a_rules['MailerLib'] = '\youconix\core\mailer\PHPMailer';
    }
  }
  ...
  protected function detectDefaults()
  {
    ...
    IoC::$a_rules['Mailer'] = '\youconix\core\services\Mailer';
