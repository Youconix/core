...
class Image extends \youconix\core\services\Upload
{
  ...
  public function __construct(\youconix\core\services\DefaultMimeTypes $defaultMimes,
                              \youconix\core\services\FileData $fileData)
  {
    parent::__construct($defaultMimes);
   ...
   * @return string
   */
  public function getExtension($s_file){
   ...
   * @return array
   */
  public function getSizes($s_file){
    $a_data = getimagesize($s_file);

    return ['width'=>$a_data[0],'height'=>$a_data[1]];
  }
  ...
  public function resizeImage($s_file, $i_maxWidth, $i_maxHeight)
  {
    ...
    if ($i_ratio != 1) {
      if ($a_mimetype[1] == 'jpg' || $a_mimetype[1] == 'jpeg') {
        $obj_trumb = $this->resize($obj_image, $s_type, $i_maxWidth,
            $i_maxHeight);
   ...
   */
  protected function resize($obj_image, $s_type, $i_desiredWidth,
                            $i_desiredHeight)
  {
    $i_width = imagesx($obj_image);
    $i_height = imagesy($obj_image);
    ...
    if (($s_type == 'gif') || ($s_type == 'png')) {
      ...
      if ($i_alpha >= 0) {
        ...
        $i_alpha = imagecolorallocate($obj_virtualImage, $a_alphaColors['red'],
            $a_alphaColors['green'], $a_alphaColors['blue']);
    ...
    imagecopyresampled($obj_virtualImage, $obj_image, 0, 0, 0, 0,
        $i_desiredWidth, $i_desiredHeight, $i_width, $i_height);
