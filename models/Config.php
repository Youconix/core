...
class Config extends Model implements \Config
{
  ...
  protected function setDefaultValues($settings)
  {
    ...
    $this->protocol = ((!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != 'off')
        || (isset($_SERVER['SERVER_PORT']) && $_SERVER['SERVER_PORT'] == 443)) ? "https://"
          : "http://";
  ...
  protected function detectAjax()
  {
    ...
    if (function_exists('apache_request_headers')) {
      ...
      $this->ajax = (isset($a_headers['X-Requested-With']) && $a_headers['X-Requested-With']
          == 'XMLHttpRequest');
    }
    if (!$this->ajax && ((isset($_GET['AJAX']) && $_GET['AJAX'] == 'true') || (isset($_POST['AJAX'])
        && $_POST['AJAX'] == 'true'))) {
      $this->ajax = true;
    }
  }
  ...
  {
    if( !$this->settings->exists('main/statistics') ){
      return true;
    }

    return ($this->settings->get('main/statistics') == '1');
  }
}